...
import { playWorkSound, playCountdownSound, playHiitSong, stopHiitSong, pauseHiitSong, resumeHiitSong, HIIT_AUDIO_CONFIG, initializeAudioPlayer, isPlayerReady, isPlayerLoading, getAudioPosition, setAudioPosition, getAudioPlayer } from '../utils/audioUtils'
...
function HiitTimer({ name = 'HIIT Workout', autoMaximize = false, autoStart = false, showBackButton = true, onBackClick }) {
  ...
  const [currentSubtitle, setCurrentSubtitle] = useState(savedState?.currentSubtitle || HIIT_CONFIG.preparation.subtitle)
  const [musicMode, setMusicMode] = useState(savedState?.musicMode ?? true) // true = YouTube music, false = beeps only
    ...
    if (musicMode && playerStatus === 'idle') {
      ...
      initializeAudioPlayer().then((ready) => {
        ...
        if (ready && savedState && savedState.audioPosition && !stateRestored) {
          ...
          console.log('🔄 Restored audio position:', savedState.audioPosition)
        }
  ...
  useEffect(() => {
    ...
    if (!isRunning && hasStarted()) {
      const audioPosition = musicMode ? getAudioPosition() : null
      ...
      console.log('💾 Saved state (paused)')
    }
  ...
  useEffect(() => {
    return () => {
      console.log('🔄 Unmounting HIIT timer...')
      ...
      if (musicMode && isRunning) {
        ...
        setTimeout(() => {
          ...
          console.log('💾 Saved HIIT state after pausing music, position:', audioPosition)
        }, 100)
      } else {
        ...
        console.log('💾 Saved HIIT state on unmount')
      }
    }
  ...
  const handleReset = () => {
    ...
    // Clear saved state
    clearHiitState()
    console.log('🗑️ Cleared saved HIIT state')
  }
    ...
    >
      ...
      {onBackClick && (
        <button
          ...
          className={`hiit-back-btn ${isMaximized ? (showBackButton ? 'visible' : 'hidden') : ''}`}
        >
          ← Back
        </button>
      )}
      ...
      {musicMode && (
        <div className="hiit-volume-control">
