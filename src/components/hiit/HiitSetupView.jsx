...
/**
 * HIIT Setup View Component
 * Configuration screen before starting workout
 */
function HiitSetupView({
  config,
  onStart,
  onBackClick,
  totalTime
}) {
  // Default values from config (we'll use fixed values for now since config has per-round timing)
  const [prepDuration, setPrepDuration] = useState(config.preparation?.duration || 10)
  const [workDuration, setWorkDuration] = useState(40) // Default HIIT work
  const [restDuration, setRestDuration] = useState(20) // Default HIIT rest

  const totalRounds = config.rounds?.length || 12
  ...
  return (
    <div className="hiit-setup-view">
      {/* Header */}
      <header className="setup-header">
        <button className="back-button" onClick={onBackClick}>
          <span className="material-symbols-outlined">arrow_back_ios_new</span>
        </button>
        <h2 className="setup-title">Workout Setup</h2>
      ...
      <main className="setup-main">
        ...
        <div className="setup-preview">
          <div className="preview-ring clickable" onClick={onStart}>
            ...
            <div className="preview-content">
              ...
              <span className="preview-time">{totalTime || '12:00'}</span>
              <span className="preview-info">{totalRounds} Sets</span>
            </div>
        ...
        <div className="interval-settings">
          ...
          <div className="interval-card">
            ...
            <div className="slider-container">
              ...
              <div
                ...
                style={{ left: `${((prepDuration - 5) / 25) * 100}%` }}
              ></div>
            </div>
          </div>
          ...
          <div className="interval-card active">
            <div className="card-header">
              <div className="card-label">
                <span className="label-category primary">HIGH INTENSITY</span>
            ...
            <div className="slider-container">
              ...
              <div
                ...
                style={{ width: `${((workDuration - 20) / 40) * 100}%` }}
              ></div>
              <div
                className="slider-thumb primary"
                style={{ left: `${((workDuration - 20) / 40) * 100}%` }}
              ></div>
            </div>
          </div>
          ...
          <div className="interval-card">
            ...
            <div className="slider-container">
              ...
              <div
                className="slider-fill"
                style={{ width: `${((restDuration - 10) / 20) * 100}%` }}
              ></div>
              <div
                className="slider-thumb"
                style={{ left: `${((restDuration - 10) / 20) * 100}%` }}
              ></div>
            </div>
          </div>
        </div>
        ...
        <div className="quote-preview">
          <div className="quote-badge">
            <span className="badge-phase">REST</span>
            <span className="badge-time">0:05</span>
          </div>
          <p className="quote-text">"Deep breaths. Get ready for the next round."</p>
        </div>
      </main>
